...
Usage:
    # Online (live capture):
    python3 pipeline.py --mode online --interface ens160
...
from pcap2text_ddos2019  import generate_digest, digest_to_prompt_text
...
def process_flow(pcap_path: str) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        validation = validate_rules(parsed["rules"], parsed["rule_type"])
    ...
    if log_entry["rule_valid"] and parsed:
        ...
        result = deploy_rules(parsed["rules"], parsed["rule_type"])
...
def run_pipeline_online(interface: str, output_dir: str, duration: int):
    ...
    flow_packets = {}   # flow_key → list of packets
    lock         = threading.Lock()
    ...
    def on_packet(pkt):
        ...
        flow_key = (f"{ip.src}_{sport}_{ip.proto}"
                    f"_{ip.dst}_{dport}")
        ...
        with lock:
            ...
            # Flow complete — write PCAP and process
            pkts      = flow_packets.pop(flow_key)
            pcap_path = os.path.join(FLOW_TMP_DIR, f"{flow_key}.pcap")
            wrpcap(pcap_path, pkts)
 
        # Process outside lock to avoid blocking capture
        entry = process_flow(pcap_path)
        with open(log_path, "a") as f:
            f.write(json.dumps(entry) + "\n")
 
    logger.info(f"[ONLINE] Sniffing on {interface} for {duration}s ...")
    sniff(
        iface=interface,
        prn=on_packet,
        store=False,
    ...
    logger.info("[ONLINE] Capture complete.")
    _print_summary(log_path)
 
 
# ── Offline mode — read PCAPs from disk ──────────────────────────────────────
 
def run_pipeline_offline(pcap_dir: str, output_dir: str):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="[online] Capture duration in seconds"
    )
    parser.add_argument(
        "--output_dir",
        default="/usr/ShieldGPT/output/online",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=YATC_THRESHOLD,
    )
    ...
    if args.mode == "online":
        ...
        run_pipeline_offline(args.pcap_dir, args.output_dir)
