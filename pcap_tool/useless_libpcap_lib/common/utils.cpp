...
void set_ipv4_pkt_info(pcap_pkthdr *pkt_header, ExtraPktInfo *extra_pkt_info, PktInfo& pkt_info,
    ...
    if (ip_header->ip_p == IPPROTO_UDP){
        ...
        extra_pkt_info->payload_start = sizeof(struct ether_header) + ip_header->ip_hl * 4 + sizeof(struct udphdr);
    }
    if (ip_header->ip_p == IPPROTO_TCP){
        ...
        extra_pkt_info->payload_start = sizeof(struct ether_header) + ip_header->ip_hl * 4 + tcp_header->th_off * 4;
    }
